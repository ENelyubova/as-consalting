...
class DCategoryTreeBehavior extends DCategoryBehavior
{
     ...
     * @return array
     */
    public function getMenuList($sub = 0, $parent = 0, $resource = "_getMenuListRecursive")
    {
        $criteria = $this->getOwnerCriteria();
        ...
        return $this->$resource($categories, $parent, $sub);
    }

    protected function _getMenuListRecursiveUser($items, $parent, $sub)
    {
        $parent = (int)$parent;
        $resultArray = [];
        if (isset($items[$parent]) && $items[$parent]) {
            foreach ($items[$parent] as $item) {
                $child = $this->_getMenuListRecursiveUser(
                    $items,
                    $item->getPrimaryKey(),
                    $sub - 1
                );
                ...
                $resultArray[$item->getPrimaryKey()] = [
                        ...
                        'active' => $active,
                        'itemOptions' => ['class' => 'listItem '. (($parent == 2) ? ' listItemParent' : '' ) . (($child) ? ' submenuItem' : '')],
